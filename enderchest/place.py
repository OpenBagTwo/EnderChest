...
from .loggers import IMPORTANT, INVENTORY_LOGGER, PLACE_LOGGER
...
def load_placement_cache(minecraft_root: Path) -> dict[str, dict[Path, list[str]]]:
    ...
    cache_file = fs.place_cache(minecraft_root)
    INVENTORY_LOGGER.debug("Loading placement cache from %s", cache_file)
    ...
    try:
        ...
        raise OSError(
            f"{cache_file} is corrupted and could not be parsed:"
        ) from decode_error
    return {
        instance_name: {
